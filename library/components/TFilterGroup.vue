<template>
  ...
  >
    ...
    <div slot="handle" class="flex items-center gap-1 p-1 text-sm font-medium">
      ...
      <span>{{
        activeFilters ? "+" + activeFilters + " Filters" : label
      }}</span>
...
window.Vue.component("expansion-wrapper", {
  render: function (createElement) {
    ...
    this.$slots.default.forEach((element, index) => {
      if (element.tag) {
        const label =
          element.data && element.data.attrs && element.data.attrs["item-label"]
            ? element.data.attrs["item-label"]
            : "-";
        const id = `${label.replace(" ", "_")}-${index}`;
        const tag_id =
          element && element.componentInstance
            ? element.componentInstance.tag_unique
            : "";
            ...
            scopedSlots: {
              default: (props) => {
                if (element.componentInstance) {
                  element.componentInstance._props.onExpandable =
                    props.onExpandable;
                }
              ...
              label: (props) => {
                return createElement(
                  "span",
                  { attrs: { id: `elem-label-${tag_id}`, ref: "label" } },
                  label
                );
              },
              icon: (props) => {
  ...
  methods: {
    ...
    handleClosed() {
      ...
      for (slotItem of this.$slots.default) {
        if (
          slotItem &&
          slotItem.componentInstance &&
          slotItem.componentInstance.$parent
        ) {
