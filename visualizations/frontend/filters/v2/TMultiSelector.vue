<template>
  ...
  <t-dropdown :is-expanded="onExpandable" :is-active="checked.length ? true : false" @open="onDropdownOpen">
    ...
    <div class="min-w-[294px]" v-bind="popupAttrs">
      <div class="px-[12px] pt-[16px] pb-[8px] flex justify-between items-center w-full">
        ...
        <span @click="selectUnselect" class="text-[#145DEB] text-[13px] cursor-pointer font-normal leading-[18px]"
          :class="checked.length ? 'text-[#145DEB]' : 'text-[#727184]'">
          <t-loading-spinner v-if="isExpanded && loading" position="relative" />
          <span v-else>
            {{ checked.length < ids.length ? 'Select All' : 'Reset' }} </span>
          </span>
      </div>
      <div class="px-2 pt-2 nav-search" v-if="isSearchable">
        <base-search-input class="mt-0 mb-3 text-sm search-report !rounded-md" :placeholder="searchPlaceholder"
          size="small" :clearable="false" v-model="search" />
      </div>
      <div class="px-[8px] pt-[4px] pb-[6px] w-full">
        <ul class="max-h-[320px] overflow-auto">
          <li class="flex justify-between px-[8px] pb-[1px] text-sm cursor-pointer text-[#555463]"
            v-for="(item, index) in menu" :key="index">
            <div class="flex items-center justify-between w-full hover:text-[#1C1C21] leading-[16.41px]">
              <base-checkbox class="!min-w-[200px]" :label="item.title" :value="item.value" v-model="checked"
