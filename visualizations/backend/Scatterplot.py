...
from common.plugin.visualization import BooleanOption, SelectOption, InputOption, ColorOption, ColorPaletteOption, VisualizationConfig, register
...
def render(source: DataFrame, config: VisualizationConfig):
    ...
    for measure in config.measures:
        ...
        if measure.options['pointShape'] == "no_point":
            ...
            scatterplotPointSize = measure.options['scatterplotPointSize']
            if is_float(scatterplotPointSize):
                marker['size'] = float(scatterplotPointSize)
            else:
                if scatterplotPointSize in source._get_numeric_data().columns:
                    print("doing size")
                    marker['size'] = source[scatterplotPointSize]
        ...
        line_trace = go.Scatter(x=x, y=y,name=seriesName,marker=marker,mode='markers')
...
register(
    ...
    options={
        ...
        'scatterplotPointSize': InputOption(
            attachment='measure',
            default='No sizing by field',
            label='Point Size',
            description='Defines the point size, which can be based on the value of another field.',
            section='Point'
        ),
