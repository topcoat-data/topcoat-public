...
def render(source: DataFrame, config: VisualizationConfig):
    ...
    seriesPositioning=config.options['seriesPositioning']
    if seriesPositioning == "stack_percentage":
        column_list = list(source)
        column_list.remove(config.dimensions[0].id_or_name)
        print(column_list)
        sum_col = source[column_list].sum(axis=1)
        
        print(source)
        i=0
        for measure in  config.measures:
            measure_series = source[measure.id_or_name]
            measure_series = (measure_series / sum_col)*100
            measure_text = measure_series.apply("{:.0f}%".format)
            x = dimension if orientation == "v" else measure_series
            y = measure_series if orientation == "v" else dimension
            showYGridLines = measure.options['showYGridLines']
            ...
            series = go.Bar(x=x, y=y, name=seriesName,marker=dict(color = seriesColor),orientation=orientation,text=measure_text)
            fig.add_trace(series,secondary_y=False)
            i+=1
        ...
        if seriesPositioning == "stack":
            fig.update_layout(barmode="stack")
        elif seriesPositioning == "group":
            fig.update_layout(barmode="group")
        elif seriesPositioning == "overlay":
        ...
        i=0
        for measure in config.measures:
            seriesChartType = measure.options['seriesChartType']
            ...
            if len(seriesColors)==0:
                ...
                seriesColor = seriesColors[i]
            if seriesChartType == 'bar_chart':
                print("x = ")
                print(x)
                print("y = ")
                print(y)
                series = go.Bar(x=x, y=y, name=seriesName,marker=dict(color = seriesColor),orientation=orientation)
                fig.add_trace(series,secondary_y=plotOnSecondaryAxis)
            else:
                ...
                marker_color = colors[i] if measure.options['pointColor'] == "auto" else measure.options['pointColor']
                lineColor = colors[i] if measure.options['lineColor'] == "auto" else measure.options['lineColor']
                line= dict(
            ...
            if plotOnSecondaryAxis:
                ...
                show_main_y_grid = show_main_y_grid | showYGridLines 
    ...
    # set bar spacing configuration
    barSpacing= float(config.options['barSpacing'])
